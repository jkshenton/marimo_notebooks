...
def _(Atoms, Path, co, file_upload, np, tempfile):
    def parse_phonon_file(uploaded_file):
        ...
        try:
            ...
            # Process q-points
            # Filter out zero-weight q-points (LO-TO splitting duplicates at Gamma)
            # CASTEP outputs Gamma twice when LO-TO splitting is calculated:
            # first with weight 1.0 (TO modes), second with weight 0.0 (LO modes)
            qpts = []
            for qpt_data in data['qpts']:
                weight = qpt_data['weight']
                # Skip zero-weight q-points (LO modes at Gamma)
                if weight == 0.0:
                    continue
...
def _(mo, phonon_data):
    ...
    if phonon_data:
        qpt_options = {
            f"q{i+1}: ({q['qpt'][0]:.3f}, {q['qpt'][1]:.3f}, {q['qpt'][2]:.3f})": i 
...
@app.cell
def _(mo, phonon_data, qpt_selector):
    ...
    if phonon_data and qpt_selector:
        ...
        mode_selector = mo.ui.dropdown(
            ...
            value=list(_mode_options.keys())[0],
            label="Phonon Mode"
        )
    else:
        mode_selector = None
...
def _(mo):
    ...
    amplitude_slider = mo.ui.slider(
        start=0.2, stop=3.0, step=0.2, value=0.6,
        label="Amplitude"
    )
...
def _(
    ...
    if phonon_data:
        mo.output.append(mo.vstack([
            ...
            mo.hstack([amplitude_slider, arrow_scale, n_frames], justify='start', gap=2),
            mo.hstack([repeat_x, repeat_y, repeat_z, arrow_color], justify='start', gap=2),
        ]))
    return
...
def _(
    ...
    if phonon_data and qpt_selector and mode_selector:
        ...
        mo.output.append(mo.md("_Upload a phonon file to visualize modes_"))
    return


@app.cell
def _(mo, np, pd, phonon_data):
    # Frequency table
    if phonon_data:
        _rows = []
        for _qpt_idx, _qpt_data in enumerate(phonon_data['qpts']):
            ...
            for _mode_idx, _freq in enumerate(_qpt_data['eigenvalues']):
                _ir = _qpt_data['ir_intensity'][_mode_idx] if _qpt_data['ir_intensity'] else np.nan
                _rows.append({
